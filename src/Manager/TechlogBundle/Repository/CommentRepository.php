...
class CommentRepository extends EntityRepository
{
    public function getList($start, $limit, $params, $params_key)
    {
		...
		$space = array('comment_id', 'article_id', 'floor', 'nickname', 'qq',
			'email', 'ip', 'reply', 'online', 'updatetime', 'inserttime');

        $total_sql = 'select count(*) as total from comment where 1';
        $data_sql = 'select '.implode(', ', $space).' from comment where 1';
